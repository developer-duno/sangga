...
pnpm test                                       # 프론트 테스트 (vitest, 397개)
pnpm test:e2e                                   # ★ 화면 E2E (playwright, 17개 × 넓은화면·휴대폰 2벌 = 34회) — 아래 경고 참조
pnpm exec oxlint                                # 프론트 린트
```

```bash
python -m pytest tests/ -q                      # 파이썬 테스트 (1,840개)
...
python scripts/check_watch_heartbeat.py         # 예약 4종이 아직 돌고 있나 (읽기만, 키 불필요 — 기준 초과면 exit 1)
