...
# 위 이름 검사는 통과하고, 그 값이 `VITE_` 를 달고 브라우저 번들에 실린다.
# 그 뒤로는 누구나 개발자도구로 그 키를 읽어 RLS 를 우회하고 전 테이블을 읽고 지운다.
#
# ⛔ **이 가드가 덮는 것은 로컬 번들뿐이다** (2026-09-01 적대검증에서 범위 정정).
#    라이브 번들은 `.env.local` 로 만들어지지 않는다 — Vercel 이 **자기 대시보드의 env** 로
#    빌드한다(`.env.local` 은 `.gitignore` 의 `.env.*` 에 걸려 레포에도 안 올라간다).
#    그러니 Vercel 콘솔의 `VITE_SUPABASE_ANON_KEY` 칸에 관리자 키를 넣는 경로는 이 검사를
#    통째로 지나간다. 그쪽은 **밖에서 묶음을 열어 보는 감시**가 잡는다 —
#    `scripts/check_live_health.py` 가 6시간마다 라이브 묶음을 받아 관리자 키 흔적을 찾는다.
#    두 검사는 서로 다른 문을 지키므로 하나로 합칠 수 없다.
#
