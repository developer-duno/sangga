...
#    유일한 방어선이다.
# ⚠️ 2026-09-09c(결정 0028) 부터 mv_search_parcel 은 **unit_business 도 읽는다**(그 땅의
#    가게 이름들). 목록·순서는 그대로지만 **갱신 시간이 는다** — 188,442 필지마다 최신
#    분기 점포를 모으기 때문이다. 그리고 새 분기를 적재하고 이걸 안 돌리면 새 가게가
#    조용히 검색에서 빠진다(에러 0 — 건물이 빠지는 것과 똑같은 방식으로).
REFRESH_MVS = (
    "mv_search_parcel",
    "mv_open_sigungu",
    "mv_sigungu_tx_stats",
...
ANON_CALLABLE_ALLOWLIST = (
    ...
    #   ⛔ 요약표 mv_search_parcel 도 여기 없다 — 그 표가 열리면 상호 묶음(store_names)이
    #      통째로 긁혀 이 함수의 상한·구 좁히기가 전부 우회된다.
    "api.search_stores",
)
