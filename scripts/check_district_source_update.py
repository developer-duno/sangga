...
# 2026-08-15 라이브 실측값. 서울은 2026-09-08 에 9/2 판을 받아 적재하고 올렸다(#128).
# ⚠️ 그 9/2 판은 8월 판과 **바이트 단위로 같았다**(zip sha256 동일, 5개 멤버 전부).
#    포털은 내용이 그대로여도 갱신일 칸을 바꿀 수 있다 — 이 알림은 "경계가 바뀌었다"가
#    아니라 "받아서 대조하라"는 뜻이고, 같더라도 절차(적재·재굽기·기준선)는 끝까지 밟는다.
# ⚠️ 새 판을 받아 적재했으면 **이 값을 올려야** 다음 갱신을 제대로 감지한다.
SBIZ_KNOWN_UPDATE = "2025-08-12"
SEOUL_KNOWN_UPDATE = "2026-09-02"
