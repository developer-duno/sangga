...
# 쓰는 근사 배수 — 최소치가 아니라 실측 기반 근사치다.
#
# ⚠️ 2026-08-10 정정: 1.7 → 1.818. 근거는 추정이 아니라 **api_quota_log 원장부**다
# (강남 3일 합계 = 표제부 12,283 · 층별개요 12,464 · 전유공용 22,317콜,
#  분모 = collect_progress done 12,274 PNU). 옛 1.7은 **데이터가 0건인 PNU에도
# 나가는 빈 응답 1콜**을 안 세어 약 7% 과소였다. 전국(1,099,599 PNU) 산정이
# 197일 → 211일로 갈리는 차이라 그냥 두면 안 된다.
#
# 교차검증: raw(expos_area.jsonl) 페이지 재계수는 22,328콜로 원장부와 11콜(0.05%)
# 차이 — 서로 다른 방법이 같은 값을 냈다.
EXPOS_CALLS_MULTIPLIER = 1.818
...
def estimate_calls(pnu_count, endpoints=ENDPOINTS):
    ...
    실측 배수(EXPOS_CALLS_MULTIPLIER, 약 1.82배 — 100건 초과 PNU의 추가 페이지에
    더해 데이터 0건 PNU의 빈 응답 1콜까지 포함)를 반영한 근사치다.

    ⚠️ 층별개요의 실측 배수는 1.0이 아니라 **1.0155**다(원장부 12,464콜 / 12,274 PNU
    — 층이 100개를 넘는 건물이 소수 있다). 여기서는 1콜로 두되, 전국 산정처럼
    큰 수를 다룰 때는 약 1.5% 과소임을 감안할 것(전국 116일 → 실제 118일).

    ⚠️ 이 함수는 현행 수집 방식(PNU마다 3종을 무조건 호출)을 전제로 센다.
    전유공용면적을 집합건물에만 호출하도록 바꾸면 이 배수는 0.953으로 떨어진다
    (강남 전수 검증: 표제부 regstrGbCdNm='집합' 판별로 놓치는 PNU 0건 —
    `docs/decisions/0005` 레버 ①). 그 변경 시 이 함수도 같이 고칠 것.
    """
    counts = Counter()
    for e in endpoints:
