...
#
# ⚠️ 2026-08-10 원본 전수 재계수로 1.7 → 1.854 정정. 옛 값은 **데이터가 0건인
# PNU에도 나가는 1콜**을 안 세어 약 9% 과소였다. 강남 3일차 완주 raw 실측:
#   표제부 PNU 11,773 / 전유공용 실호출 21,827콜
#     = 데이터 있는 1,646 PNU의 페이지 11,700콜 + 빈 응답 1콜 × 10,127 PNU
# 전국(1,099,599 PNU) 산정이 197일 → 215일로 갈리는 차이라 그냥 두면 안 된다.
EXPOS_CALLS_MULTIPLIER = 1.854
...
def estimate_calls(pnu_count, endpoints=ENDPOINTS):
    ...
    실측 배수(EXPOS_CALLS_MULTIPLIER, 약 1.85배 — 100건 초과 PNU의 추가 페이지에
    더해 데이터 0건 PNU의 빈 응답 1콜까지 포함)를 반영한 근사치다.

    ⚠️ 이 함수는 현행 수집 방식(PNU마다 3종을 무조건 호출)을 전제로 센다.
    전유공용면적을 집합건물에만 호출하도록 바꾸면 이 배수는 약 0.99로 떨어진다
