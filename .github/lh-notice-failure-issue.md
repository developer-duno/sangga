...
- **연결 실패(가장 흔함)** — GitHub 러너(미국)에서 한국 포털(data.go.kr)로 가는 연결이
  간헐적으로 막힙니다. 스크립트는 60초 타임아웃으로 **5번까지**(사이사이 5·10·20·40초를
  쉬며 총 약 6분) 다시 시도하므로, 그걸 넘겼다면 포털이 몇 분 넘게 닫혀 있었다는 뜻입니다.
  **이 경우 내 PC 에서 위 명령이 그냥 되는 일이 흔합니다** — 되고 "새 공고: 없음" 이면
  놓친 것이 없으니 이 이슈를 닫으면 끝입니다(2026-08-31 첫 실패가 정확히 이 형태였습니다).
