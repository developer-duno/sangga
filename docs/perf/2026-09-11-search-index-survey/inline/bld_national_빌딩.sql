...
with pat as (
    ...
    select pc.pnu, pc.road_addr, pc.jibun_addr_key
     ...
     where '%빌딩%' is not null
       ...
       and (pc.road_addr_key  like '%빌딩%' escape '\'
         or pc.jibun_addr_key like '%빌딩%' escape '\')
  ...
  eligible as (
    select h.*,
           ...
           coalesce(h.jibun_addr_key like '%빌딩' escape '\', false)
             as jibun_hit
    from hit h
  ...
  top as (
    ...
    order by
      e.jibun_hit                    desc,
      (e.nm_key = '빌딩')             desc nulls last,
      (e.nm_key like '빌딩' || '%')   desc nulls last,
  ...
  order by
    t.jibun_hit                     desc,
    (t.nm_key = search_key('빌딩'))      desc nulls last,
    (t.nm_key like search_key('빌딩') || '%') desc nulls last,
