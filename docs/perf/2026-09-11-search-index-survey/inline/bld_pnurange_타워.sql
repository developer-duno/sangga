...
with pat as (
    select
      ...
      esc.gu
    from (
      select case when search_key('타워') is null then null
                  else replace(replace(replace(search_key('타워'), '\', '\\'),
                               '%', '\%'), '_', '\_')
             end as v,
  ...
  addr as materialized (
    select pc.pnu, pc.road_addr, pc.jibun_addr_key
     ...
     where '%타워%' is not null
       ...
       and (pc.road_addr_key  like '%타워%' escape '\'
         or pc.jibun_addr_key like '%타워%' escape '\')
  ...
  eligible as (
    select h.*,
           ...
           coalesce(h.jibun_addr_key like '%타워' escape '\', false)
             as jibun_hit
    from hit h
  ...
  top as (
    ...
    order by
      e.jibun_hit                    desc,
      (e.nm_key = '타워')             desc nulls last,
      (e.nm_key like '타워' || '%')   desc nulls last,
  ...
  order by
    t.jibun_hit                     desc,
    (t.nm_key = search_key('타워'))      desc nulls last,
    (t.nm_key like search_key('타워') || '%') desc nulls last,
