...
with pat as (
    select
      ...
      esc.gu
    from (
      select case when search_key('플라자') is null then null
                  else replace(replace(replace(search_key('플라자'), '\', '\\'),
                               '%', '\%'), '_', '\_')
             end as v,
  ...
  addr as materialized (
    select pc.pnu, pc.road_addr, pc.jibun_addr_key
     ...
     where '%플라자%' is not null
       ...
       and (null::text is null or pc.sigungu_code = null::text::char(5))
       and (pc.road_addr_key  like '%플라자%' escape '\'
         or pc.jibun_addr_key like '%플라자%' escape '\')
  ...
  eligible as (
    select h.*,
           ...
           coalesce(h.jibun_addr_key like '%플라자' escape '\', false)
             as jibun_hit
    from hit h
  ...
  top as (
    ...
    order by
      e.jibun_hit                    desc,
      (e.nm_key = '플라자')             desc nulls last,
      (e.nm_key like '플라자' || '%')   desc nulls last,
      (e.bld_nm is null)             asc,
      length(e.bld_nm)               asc nulls last,
      e.road_addr                    asc nulls last,
      e.bld_id
    limit greatest(1, least(coalesce(25, 25), 100))
  )
  ...
  order by
    t.jibun_hit                     desc,
    (t.nm_key = search_key('플라자'))      desc nulls last,
    (t.nm_key like search_key('플라자') || '%') desc nulls last,
