  ...
  hit as (
    select m.pnu, p.road_addr, m.store_names, m.store_snapshot_ym
     ...
     where '%학원%' is not null
       ...
       and m.store_names_key like '%학원%' escape '\'
    ...
    select h.pnu, h.road_addr, h.store_snapshot_ym,
      ...
      join lateral (
        select count(*)::int as n,
               coalesce(bool_or(search_key(u.nm) = '학원'), false) as exact_hit
          from unnest(h.store_names) as u(nm)
         where search_key(u.nm) like '%학원%' escape '\'
      ) agg on true
    ...
    join lateral (
      ...
      from (
        select distinct u2.nm,
               (search_key(u2.nm) = '학원') as is_exact
          from mv_parcel_store_names m2,
               unnest(m2.store_names) as u2(nm)
         where m2.pnu = pg.pnu
           and search_key(u2.nm) like '%학원%' escape '\'
