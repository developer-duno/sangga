...
with pat as (
    ...
    select
      ...
      search_key('스타벅스') as k,
      esc.gu
    from (
      select case when search_key('스타벅스') is null then null
                  else replace(replace(replace(search_key('스타벅스'), '\', '\\'),
                               '%', '\%'), '_', '\_')
             end as v,
  ...
  hit as (
    select m.pnu, p.road_addr, m.store_names, m.store_snapshot_ym
      ...
      cross join pat
     where '%스타벅스%' is not null
       and '11680' is not null
       ...
       and m.sigungu_code = '11680'::char(5)
       and m.store_names_key like '%스타벅스%' escape '\'
    ...
    select h.pnu, h.road_addr, h.store_snapshot_ym,
      ...
      join lateral (
        select count(*)::int as n,
               coalesce(bool_or(search_key(u.nm) = '스타벅스'), false) as exact_hit
          from unnest(h.store_names) as u(nm)
         where search_key(u.nm) like '%스타벅스%' escape '\'
      ) agg on true
    ...
    join lateral (
      ...
      from (
        select distinct u2.nm,
               (search_key(u2.nm) = '스타벅스') as is_exact
          from mv_parcel_store_names m2,
               unnest(m2.store_names) as u2(nm)
         where m2.pnu = pg.pnu
           and search_key(u2.nm) like '%스타벅스%' escape '\'
