...
/**
 * **가게 이름**으로 그 가게가 있는 **땅**을 찾는다(`{ q, lim, sigungu, p_offset }`). 결정 0028.
 *
 * ⓘ 위와 같은 이유로 이름만 적는다(`api.search_stores` 로 간다).
 * ⛔ **건물 검색과 별개의 함수다** — `search_buildings` 에 상호 가지를 더하면 가지 2→3 에
 *    763→1,550ms 가 된다(알려진한계 §4). 두 함수를 나란히 부르고 결과는 두 구역으로 나눈다.
 * ⛔ 한 줄이 **건물이 아니라 땅(필지)** 이다 — 점포를 필지 단위로만 셀 수 있어서다
 *    (`unit_business.unit_id` 전량 NULL). 건물로 줄세우면 한 땅의 동들이 같은 가게를
 *    **복사해** 갖는다(결정 0025 의 그 함정).
 * ⛔ 검색어가 너무 넓으면 **0건이 아니라 `too_broad=true` 한 줄**이 온다 — 0건("그런 가게가
 *    없다")과 갈라 대해야 하는 다른 상태다. 지금의 `search_scope` 는 상호를 안 세므로
 *    그쪽을 고쳐 쓸 수 없다(고치면 "기존 함수 불변"이 깨진다).
 * ⓘ 함수가 아직 없으면(PGRST202) 화면은 **구역을 통째로 생략**한다. 그 밖의 실패는 생략하지
 *   않고 "불러오지 못했습니다"라 적는다 — 사람이 누른 결과가 조용히 사라지면 안 된다(0025 규칙).
 */
export const SEARCH_STORES_FN = 'search_stores';

/**
 * 가게 이름 결과를 한 번에 몇 곳(땅)씩 받아 올지. 👤 사장님 결재(2026-09-09, 결정 0028 결정 2):
 * "50줄 + 더 보기" — 상권 건물 목록(`DISTRICT_BUILDINGS_PAGE`)과 **같은 값**이다.
 *
 * ⛔ 그렇다고 그 상수를 함께 쓰지 않는다 — 두 목록은 서버 함수도 결재 근거도 다르므로,
 *    한쪽 쪽수를 바꾸는 날 다른 쪽이 말없이 따라 움직이면 안 된다.
 * ⚠️ 서버가 상한 200 으로 한 번 더 막는다. 이 값을 200 위로 올리면 **에러 없이 조용히
 *    200 으로 깎인다** — 늘리려면 서버 쪽 상한도 같은 커밋에서 함께 올린다(형제와 같은 규칙).
 */
export const SEARCH_STORES_PAGE = 50;
...
/**
 * 구 실거래 단가의 **연도별** 흐름(결정 0027). 인자는 `sigungu`(5자리다).
 *
 * ⚠️ 인자 이름을 여기 적어 두는 이유 — 목(mock)은 인자 **이름**을 안 보므로 딴 이름으로
 *    잘못 불러도 vitest·E2E 는 전부 초록이고 **라이브에서만** PGRST202 가 난다
 *    (`list_price_bands` 의 `p_pnu` 에서 실제로 겪은 함정이다).
 *
 * 자료가 있는 해만 오고(빈 해를 0 으로 채우지 않는다) 연도 오름차순이다. 자료 범위
 * (첫 달·끝 달)도 서버가 함께 주므로, 화면은 그 범위를 글자로 박지 않는다.
 */
export const SIGUNGU_TX_YEARLY_FN = 'get_sigungu_tx_yearly';

/**
 * 실거래 통계의 **미표시 임계값**(건수).
 *
 * 이보다 표본이 적으면 중앙값·사분위를 적지 않는다 — 몇 건으로 낸 가운데값은 숫자 모양만
 * 통계이고, 화면에 적히는 순간 사람은 그걸 근거로 쓴다(검증 규칙의 미표시 원칙).
 *
 * ⚠️ 층별 화면(`FloorStack`)과 입구의 흐름 카드가 **같은 값**을 써야 한다. 두 곳에 5 를
 *    따로 적어 두면 한쪽만 고치는 날 같은 자료를 두 화면이 다르게 가린다.
 */
export const TX_MIN_SAMPLE = 5;
...
/**
 * 참고 시세 게이트의 **구별 판정과 그 근거**(인자 없음). 2026-09-05e — 로드맵 Wave 4.
 *
 * ⛔ **통과 구 목록을 화면·문서에 복사하지 않는다**(결정 0013 §4). 목록의 진실은 표
 *    `price_gate_sigungu` 한 곳이고, 성적표를 다시 뽑아 적재하면 화면이 저절로 따라온다 —
 *    이 함수는 그 표를 **목록으로** 읽으라고 뚫어 둔 유일한 길이다.
 *    (형제 `list_price_bands` 는 같은 표를 보지만 "이 구가 통과했나" 하나만 쓴다.)
 *
 * ⓘ 돌려주는 것은 **열린 구마다 한 줄**이다 — 통과한 구만 오는 것이 아니다. 떨어진 구도
 *   와야 "왜 우리 구는 참고 시세가 없나"에 답할 수 있고, 그 답이 이 카드의 본론이다.
 * ⓘ 함수가 아직 없으면(PGRST202) 화면은 **카드를 통째로 생략**한다(LH 공고와 같은 규칙).
 */
export const PRICE_GATE_FN = 'list_price_gate';

/**
 * 성적표를 구워 둔 정적 파일. `public/` 에 있어 번들에 안 들어간다.
 *
