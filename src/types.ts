...
export type SigunguTxYearly = {
  ...
  p75_unit_price: number | null;
  /**
   * 그 해 단가의 **근거가 된 거래 한 건**의 건물면적 중앙값(㎡) — 위 단가와 같은 모집단에서
   * 잰 값이다(2026-09-09b).
   *
   * ⚠️ **선택 칸이다.** 마이그레이션을 적용하기 전 라이브의 함수는 이 칸을 안 준다 —
   *    그때는 이 조각만 조용히 빠지고 나머지 줄은 그대로 선다(칸이 없다고 카드를 통째로
   *    버리면, 사실 한 칸을 더하려다 있던 카드를 없애는 셈이 된다).
   * ⛔ 숫자를 거르는 칸이 아니다 — 다른 해보다 유난히 작으면 초소형 구획이 무더기로
   *    거래된 해라는 것을 사람이 알아보게 하는 사실 한 칸이다.
   */
...
export type NearbyPermits = {
  ...
  started_cnt: number;
  /**
   * 그중 **기준월 말일 기준으로 허가 후 2년 넘게 착공 기록이 없는** 것(마이그레이션 2026-09-05b).
   *
   * ⛔ **"허가가 실효됐다"는 뜻이 아니다.** 원본(건축HUB 기본개요)에 실효 상태 칸이 아예
   *    없어서, 우리가 아는 것은 "그 달 자료에 착공 기록이 없다"까지다. 화면 문장도 거기까지만
   *    말한다(사장님 결재 2026-09-05).
   * ⚠️ **선택 칸이다.** 2026-09-05b 를 아직 안 올린 라이브는 이 칸을 안 준다 — 그때는 그
   *    문장만 조용히 빠지고 나머지 줄은 그대로 선다.
   * ⚠️ `total_cnt` 에서 빼는 수가 아니라 **그 안에 들어 있는 부분집합**이다. 착공한 것은
   *    세지 않으므로 `total_cnt - started_cnt` 를 넘을 수 없다.
   */
  stale_cnt?: number;
...
export type LhNotice = {
   ...
   *
   * ⚠️ **null 일 수 있다.** DB 컬럼이 nullable(text, NOT NULL 없음)이고, 수집기
   *    (`record_to_row`)가 빈 값을 NULL 로 적는다 — 즉 **원본이 이 칸을 비워 주면
   *    NULL 이 들어온다.** null 이면 화면은 그 칸만 비우고 나머지 줄은 그대로 그린다.
   *    ⓘ 지금 라이브에는 NULL 이 **0건**이다(531행 실측, 2026-09-01). 그래서 이건
   *      "지금 나는 사고"가 아니라 **잠복**이다 — 목록 검증이 `.every()` 라 그런 행이
   *      하나만 섞여도 카드가 **통째로** 사라지므로, 나기 전에 막아 둔다.
   *      (초안에는 "그런 행이 실제로 있다"고 적었는데 라이브를 안 재고 쓴 말이었다 —
   *       2026-09-01 독립 검토에서 정정.)
   */
  pan_ss: string | null;
   ...
   *
   * ⚠️ **null 일 수 있다** — `pan_ss` 와 같은 이유(DB nullable + 원본 결측). null 이면
   *    `isHttpUrl` 이 걸러 링크를 아예 안 만든다(주소 모양이 아닌 값과 같은 처리).
   */
  dtl_url: string | null;
  /** 우리가 이 줄을 받아 둔 시각. 요약 줄에 그대로 밝힌다 — 낡음을 감추지 않는다. */
  collected_at: string;
  /**
   * **LH 가 지역을 '전국'이라 적었나**(결정 0026 · 마이그레이션 2026-09-05c).
   *
   * ⛔ '지역 칸이 비었다'는 뜻이 **아니다** — 빈 칸은 이 값이 false 로 들어온다
   *    (수집기 `map_sido` 실측). 즉 이건 결측이 아니라 **LH 가 적어 준 말**이다.
   * ⚠️ 그런데 그렇게 적힌 공고의 제목은 인천계양·부산명지처럼 **특정 도시**인 경우가
   *    대부분이다(2026-09-05 실측: 서울 39줄 중 35줄). 그래서 화면은 이 줄들을 접어
   *    두고, **제목을 보고 지역을 추측하지는 않는다**(틀린 지역표는 없느니만 못하다).
   * ⓘ **선택 칸이다** — 05c 이전 라이브는 이 값을 안 준다. 그때는 전부 지역 공고로
   *   보고 지금까지처럼 한 목록에 늘어놓는다(화면이 조용히 옛 모습으로 돌아간다).
   *   `null` 도 같은 뜻으로 받는다 — 검증기가 null 을 통과시키므로(한 줄 때문에 카드가
   *   통째로 사라지지 않게) 타입도 그 사실을 그대로 적는다.
   */
  is_nationwide?: boolean | null;
  /**
   * 같은 공고가 **나 말고 몇 줄 더 있었나**(0 이면 재게시 없음).
   *
   * 대괄호 표시([정정공고] 등)와 공백을 지운 제목이 같은 줄을 서버가 하나로 묶고,
   * 그중 최신 한 줄에 이 수를 실어 준다. 화면은 0보다 클 때만 "정정·재게시 N회"를 적는다.
   * ⓘ `is_nationwide` 와 같은 이유로 **선택 칸**이고 `null` 도 받는다(05c 이전 라이브는
   *   안 준다). null 이면 꼬리표를 안 붙인다.
   */
  dup_cnt?: number | null;
};
...
//    서버가 준다 — 화면에 글자로 박아 두면 자료가 하나 늘거나 주기가 바뀌는 날 화면만
//    옛말을 한다(각주 숫자를 `v_coverage_stats` 로 옮긴 것과 같은 이유).

/** 함수 `get_data_freshness()`의 한 줄 = 자료 한 갈래. */
export type DataFreshnessRow = {
  /** 자료 이름('점포·업종 (상권정보)'). 화면이 그대로 적는다. */
  src: string;
  /**
   * 기준값이 **무엇인가** — '분기' · '계약월' · '기준월' · '고시일' · '적재일' ·
   * '계산일' · '수집일' · '갱신일'.
   *
   * ⚠️ 타입을 그 여덟으로 **좁히지 않는다**(`LhNotice.kind_nm` 과 같은 이유). 화면은
   *    이 글자를 그대로 적고, 값을 어떻게 읽을지만 앞의 세 갈래(분기·달·날짜)로 가른다.
   */
  basis_kind: string;
  /**
   * 기준값 그 자체 — '202606' · '2026Q2' · '2026-08-27'.
   *
   * ⚠️ **null 일 수 있다.** 그 자료가 아직 한 행도 없다는 뜻이고, 그때도 줄은 그대로
   *    나온다(화면이 '자료 없음'이라 적는다). 줄을 빼면 "그런 자료를 안 쓴다"로 읽힌다.
   */
  basis: string | null;
  /**
   * 다음 갱신 예정일('2026-10-31'). 서버가 **규칙으로 계산**한 값이다.
   *
   * ⛔ **null 이 정상인 자료가 절반이 넘는다** — 실거래·건축물대장·상권 경계·LH·성적표·
   *    필지는 정해진 주기가 없다. 없는 주기를 지어내면 "늦었다"는 거짓 신호가 뜬다.
   */
