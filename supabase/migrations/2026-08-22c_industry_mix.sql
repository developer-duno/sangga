...
-- ─────────────────────────────────────────────────────────────────────
-- ④(469ms)도 최악(강남 845필지·4,659점포)에서는 **1,583ms** 였다. 원인은 `idx_ub_pnu`
-- 가 행 위치만 알려 줘서, 업종 칸을 읽으려고 **점포마다 힙을 한 번씩** 뒤지기 때문이다.
-- 업종 네 칸을 인덱스 안에 넣어 두면 힙에 갈 일이 없다(2026-08-11 `idx_ub_snapshot_floor_pnu`
-- 와 **같은 처방**). 트랜잭션 안에서 만들었다 되돌리며 실측했다:
--
--   반경 최악 대분류 집계 : 1,583ms → **23ms** (Heap Fetches 0, 버퍼 878읽기 → 73)
--   중분류 상세          :          → **9.9ms**
--
...
--
-- 검산 (실행 후):
--   explain (analyze, buffers) select cat_l_cd, count(*) from unit_business
--    where snapshot_ym = (select max(snapshot_ym) from unit_business)
--      and pnu = any(array['1168010100106010000']::char(19)[]) group by 1;
--     → "Index Only Scan using idx_ub_pnu_cat" 와 **Heap Fetches: 0** 이 보여야 한다.
...
-- 옛 인덱스는 위 인덱스의 앞부분이라 중복이다(2026-08-22a 와 같은 정리).
