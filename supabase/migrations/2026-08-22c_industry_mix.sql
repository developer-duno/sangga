...
--   적용 직후, 새 인덱스 : **44.5ms** — `list_industry_mix()` 한 번을 통째로 잰 값
--                          (`list_industry_detail()` 46.3ms). 반경 집계만 떼어 보면
--                          **14.7ms · Heap Fetches 0 · Index Only Scan** (2026-08-22 라이브)
--
-- ⚠️ 위 "적용 직후"는 인덱스를 막 만들고 vacuum 을 돌린 뒤라 **그 페이지가 아직 메모리에
--    남아 있는** 상태다. 진짜 찬 캐시는 이보다 느릴 수 있다 — 다만 힙 방문이 0 이라
--    옛것처럼 점포 수만큼 랜덤 읽기가 생기지는 않는다(그게 2,383ms 의 원인이었다).
-- ⚠️ 더운 캐시로는 이 차이가 아예 안 보인다. 같은 조회가 옛 인덱스로도 **23ms** 로 끝난다
