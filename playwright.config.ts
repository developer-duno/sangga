...
export default defineConfig({
  testDir: './e2e',
  // ⛔ `test.only` 가 하나라도 섞여 들어오면 CI 를 **실패**시킨다.
  //
  // playwright 는 `.only` 가 있으면 그것만 돌리고 나머지를 통째로 건너뛴다 — 54회가 1~2회로
  // 줄어드는데 결과는 **초록**이다. 디버깅하다 `.only` 를 지우는 것을 잊은 채 올리면, 그 PR 도
  // 그 뒤의 모든 PR 도 "E2E 통과"라고 말하면서 실제로는 아무것도 안 본다. 이 레포가 가장
  // 자주 데인 것이 바로 이 "가짜 초록"이라(경쟁상태 회귀시험·CSS 빈 문자열 등) 기계로 막는다.
  // 로컬에서는 끄고(디버깅 중엔 `.only` 가 편하다) CI 에서만 켠다.
